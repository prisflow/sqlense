...
export default function VSCodeViewer({ studentId, studentName, open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/dashboard/student-by-no/${studentId}`, { signal: AbortSignal.timeout(5000) })
      ...
      .then((d) => { if (d.student) { const base = import.meta.env.VITE_IDE_BASE_URL || `//${window.location.hostname}`; setIdeUrl(`${base}/student-${d.student.student_no}/`); } else throw new Error("no student"); })
